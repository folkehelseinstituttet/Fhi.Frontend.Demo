...
export class FhiTreeViewSelectionComponent implements OnInit, OnChanges {
  ...
  ngOnChanges() {
    if (this.items !== undefined) {
      this.items = koder;
      console.log('Items er:', this.items);
      this.createIds(this.items, 1);
      const retState = this.updateDecendantState(this.items, true);
      console.log('ReturnedState er:', retState);
    }
  }
   ...
   * @param initialState
   */
  private updateDecendantStateOld(items: Item[], initialState: boolean) {
    console.log('iterasjon:', this.count++);
    if (this.count > 10) return;
    items.forEach((item) => {
      console.log('sjekker item', item.name);
      if (item.children && item.children.length > 0) {
        ...
        this.updateDecendantStateOld(item.children, initialState);
      } else if (!item.descendantStateConfirmed) {
        item.descendantStateConfirmed = true; // I.e. has no descendants
        this.updateDecendantStateOld(this.items, initialState);
      }
  ...
  private updateDecendantState(items: Item[], initialState: boolean): FhiTreeViewSelectionItemState {
    const myIt = this.count;
    const myState =  {
    ...
    items.forEach((item) => {
      ...
      if (item.children && item.children.length > 0) {
        // item har barn
        if (item.children.every((item) => item.descendantStateConfirmed)) {
          // alle barna til item er confirmed
          if (item.children.find((item) => item.isChecked) !== undefined) {
            // det finnes noen som er huket av
            this.setHasCheckedDescendantAndIsExpanded(true, item, initialState);
          } else {
            this.setHasCheckedDescendantAndIsExpanded(false, item, initialState);
          }
        ...
        if (!item.descendantStateConfirmed) {
          const childrensState = this.updateDecendantState(item.children, initialState);
          item.hasCheckedDescendant = item.hasCheckedDescendant || childrensState.hasCheckedDescendant;
        ...
        if (initialState && item.isChecked) {
          console.log('!!!!!!!!!!!! Har satt isExpanded for ', item.id, ' navn:', item.name);
          item.isExpanded = true;
        }

        item.descendantStateConfirmed = true;
      }
      if (!myState.hasExpandedDescendant && item.isExpanded !== undefined) { myState.hasExpandedDescendant = item.isExpanded}
      if (!myState.hasCheckedDescendant && item.isChecked !== undefined) { myState.hasCheckedDescendant = item.isChecked}
    });
    console.log('iterasjon %s ferdig.', myIt, 'min status for alle søsken på dette nivået er:', myState);
    return myState;
  }
  ...
  private createIds(items: Item[], id: number) {
    items.forEach((item) => {
      ...
      if (item.id === undefined) {
        item.id = id++;
        console.log('%s->%s', item.name, item.id);
      }
      if (item.children && item.children.length > 0) {
