...
| `unitId` | `number \| string`      | -       | no       | Used to associate the serie with a unit. This only works if `unit.id` is set, and value is equal to `serie.unitId`. See [FhiDiagramUnit](#interface-fhidiagramunit) for details. |
...
| Property   | Type               | Default | Required | Description |
| ---------- | ------------------ | ------- | -------- | ----------- |
| `id`       | `number \| string` | -       | no       | Used to associate the unit with a serie. This only works if a `serie.unitId` is set, and value is equal to `unit.id`. See [FhiDiagramSerie](#interface-fhidiagramserie) for details. |
